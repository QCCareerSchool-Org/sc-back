...
export type GetNewAssignmentResponseDTO = NewAssignmentDTO & {
  newSubmission: Omit<NewSubmissionDTO, 'complete' | 'points' | 'mark' | 'markOverride'>;
...
export class GetNewAssignmentInteractor implements IInteractor<GetNewAssignmentRequestDTO, GetNewAssignmentResponseDTO> {
  ...
  public async execute({ assignmentId }: GetNewAssignmentRequestDTO): Promise<ResultType<GetNewAssignmentResponseDTO>> {
    try {
      ...
      return Result.success({
        ...
        newParts: assignment.newParts.map(p => {
          ...
          const partDTO = {
            ...
            points: partPoints,
            mark: assignment.newSubmission.closed ? partMark : null,
            markOverride: assignment.newSubmission.closed ? partOverridden ? partMarkOverride : null : null,
          };
          if (!partComplete) {
        ...
        markOverride: assignment.newSubmission.closed ? assignmentOverridden ? assignmentMarkOverride : null : null,
        ...
        newSubmission: {
          ...
          markingCriteria: assignment.newSubmission.markingCriteria,
